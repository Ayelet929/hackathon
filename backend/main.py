...
async def handle_user_query(user_query: QueryRequest):
    ...
    try:
        ...
        full_prompt = f"אתה מומחה לזוגיות. שאלו אותך שאלה  : {query_string} תענה בצורה ממוקדת וקצרה , ניתן להשתמש במידע נוסף על המשתמש אם יש אבל לענות בצורה חכמה בלי שירגיש שאתה יודע את המידע  "
